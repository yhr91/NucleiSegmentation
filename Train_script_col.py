...
## Deal separately with color images
...
for img, maps in zip(X_train_col, X_train_col_maps):
    X_train_col_patches.append(get_patches(img, patch_size))
    
    maps = np.max(np.array(maps),0)
    X_train_col_maps_patches.append(get_patches(maps, patch_size))
...
checkpointer = ModelCheckpoint('model-dsbowl2018-col_1.h5', verbose=1, save_best_only=True)
